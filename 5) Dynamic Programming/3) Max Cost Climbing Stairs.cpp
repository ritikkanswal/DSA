// https://leetcode.com/problems/max-cost-climbing-stairs/
...
public:
    ...
    int maxCostClimbingStairs(vector<int>& cost) {
        ...
        return max(fun(cost,0),fun(cost,1));
    }
};
...
public:
    ...
    int maxCostClimbingStairs(vector<int>& cost) {
        int n = cost.size();
        return max(helper2(cost , n-1) , helper2(cost , n-2));
    }
};
...
public:
    int helper3(vector<int>& cost, int n){
        ...
        for(int i = 2; i < n; i++){
            int temp = cost[i] + max(prev2 , prev1);
